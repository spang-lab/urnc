...
example_course_config = f"{example_course}/config.yaml"


def setup_module(module):
    """Make sure the used test course is a git repo, as we don't commit the .git
    folders from the test courses. This function is run once before all tests in
    this module."""
    ...
    # Remove ACCESS_TOKEN mentioned in example_course_config, as it most likely
    # is not set in the environment of the person running the tests. Since we
    # only need to clone/pull/checkout during the tests, but not push, this is
    # fine.
    yaml = open(example_course_config).read()
    yaml = yaml.replace("urncbot:{URNC_ACCESS_TOKEN_STUDENT_REPO}@", "")
    open(example_course_config, "w").write(yaml)
